...
On regarde cette algorithme plus en détail sur le calcul de la trace modulo $l$.\\
...
\begin{remarque}
Pour résumer, si un $\tau$ vérifie \eqref{abscisse} et \eqref{ordonnée}, alors la trace est égale à $\tau$ modulo $l$. Sinon si aucun $\tau$ ne vérifie les deux relation, alors la trace est nulle modulo $l$.
\end{remarque}
...
\begin{itemize}
\item Si $\forall P$ nonzéro \begin{equation}
\phi_{l}^{2}P = \pm kP
\label{carré}
\end{equation} avec $q\equiv k[l]$ 
\item Sinon on fait le cas général.
\end{itemize}

On va donc transformer l'équation \eqref{carré} en un calcul de $pgcd$ pour pouvoir l'implémenter sur machine.
